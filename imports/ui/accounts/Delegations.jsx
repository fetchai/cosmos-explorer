                    ...
                    <SentryBoundary>
                        {this.props.delegations.sort((b, a) => (a.balance - b.balance)).map((d, i) => {
                            ...
                            return <Row key={i} className="delegation-info">
                                ...
                                <Col xs={2} md={5} className="overflow-auto">{new Coin(d.balance.amount, denomType).toString()}</Col>
                                <Col xs={3} md={3}>{rewardDenom ? new Coin(rewardDenom.amount, rewardDenom.denom).toString() : 'No rewards '} </Col>
                            </Row>
                        })}</SentryBoundary>
                </Container>
            </CardBody> : ''}
