...
export default class AccountTooltip extends Account {
  ...
  renderDetailTooltip() {
    ...
    const moniker = validator.description && validator.description.moniker || validator.address;
    const isActive = validator.status == 2 && !validator.jailed;

    return (
      <UncontrolledPopover className="validator-popover" trigger="hover" placement="right" target={this.ref}>
        <Card className="validator-popover-card" body outline color="danger">
          <PopoverHeader>
            <CardTitle className="d-flex justify-content-center">
              <h4 className="moniker text-primary">
                {moniker}
              </h4>
            </CardTitle>
          </PopoverHeader>
          <PopoverBody>
            ...
            {(isActive) ? (
              <CardText className="commission data">
            ...
            {(!isActive) ? (
              <CardText className="last-seen data">
            ...
            {(!isActive) ? (
              <CardText className="bond-status data" xs={2}>
                <Col xs={6}>
                  {(validator.status == 0) ? <Badge color="secondary">Unbonded</Badge> : <Badge color="warning">Unbonding</Badge>}
                </Col>
                <Col xs={6}>
                  {validator.jailed ? <Badge color="danger">Jailed</Badge> : ''}
                </Col>
              </CardText>
            ) : null}
            {(isActive) ? (
              <CardText className="uptime data">
    ...
    return [
      <span ref={this.ref} key="link" className="validator-popover-row">
        <Link to={this.state.address}>
          <Avatar moniker={this.state.moniker} profileUrl={this.state.validator ? this.state.validator.profile_url : ''} address={this.state.address} />
          {' '}
          {this.state.moniker}
          {' '}
        </Link>
