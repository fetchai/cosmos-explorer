...
export default class SearchBar extends Component {
  constructor(props) {
    ...
    redirectSearchResult = (query) => {
      ...
      const validatorRegEx = new RegExp(`${Meteor.settings.public.bech32PrefixValAddr}.*$`, 'igm');
      const accountRegEx = new RegExp(`${Meteor.settings.public.bech32PrefixAccAddr}.*$`, 'igm');
      if (query != '') {
        if (!isNaN(query)) {
    ...
    render() {
      return (
        <InputGroup className={(this.props.mobile) ? 'd-lg-none' : 'd-none d-lg-flex'} id={this.props.id}>
          ...
          {(this.props.mobile) ? (
            <InputGroupAddon addonType="append">
              <Button>
                <i className="material-icons" onClick={this.handleMobileSearch}>search</i>
              </Button>
            </InputGroupAddon>
          ) : ''}
        </InputGroup>
