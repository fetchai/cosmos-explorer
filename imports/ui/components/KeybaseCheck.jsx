...
export default class KeybaseCheck extends React.Component {
  ...
  render() {
    ...
    if (this.state.verified) {
      return (
        <a href={this.state.keybaseUrl} target="_blank">
          {(this.props.showKey ? `${this.props.identity} ` : '')}
          <i className="fas fa-check-circle text-success" title={this.props.identity} />
        </a>
      );
    }
    ...
    return (
      <span>
        {(this.props.showKey ? `${this.props.identity} ` : '')}
        <i className="fas fa-check-circle text-warning" title="Keybase identity not verified." />
      </span>
    );
  }
}
