    ...
    if (this.props.validatorExist) {
      return (
        <div>
          ...
          <Link to={`/validator/${this.props.validator.address}`} className="btn btn-link">
            <i className="fas fa-caret-left" />
            {' '}
            <T>validators.backToValidator</T>
          </Link>
          <h2>
            <T moniker={this.props.validator.description.moniker}>validators.missedBlocksTitle</T>
          </h2>
          <Nav pills>
            <NavItem>
              <NavLink tag={Link} to={`/validator/${this.props.validator.address}/missed/blocks`} active={this.isVoter()}>
                <T>validators.missedBlocks</T>
              </NavLink>
            </NavItem>
            <NavItem>
              <NavLink tag={Link} to={`/validator/${this.props.validator.address}/missed/precommits`} active={!this.isVoter()}>
                <T>validators.missedPrecommits</T>
              </NavLink>
            </NavItem>
          </Nav>
          {(this.props.missedRecords && this.props.missedRecords.length > 0)
            ? (
              <div className="mt-3">
                <p className="lead">
                  <T>validators.totalMissed</T>
                  {' '}
                  {this.isVoter() ? <T>common.blocks</T> : <T>common.precommits</T>}
                  :
                  {' '}
                  {this.props.missedRecords.length}
                </p>
                <CardDeck>
              ...
              <div>
                <T>validators.iDontMiss</T>
                {this.isVoter() ? <T>common.block</T> : <T>common.precommit</T>}
                .
              </div>
            )}
          {this.props.statusExist ? (
            <div>
              <em>
                <T>validators.lastSyncTime</T>
                :
                <TimeStamp time={this.props.status.lastMissedBlockTime} />
              </em>
            </div>
          ) : ''}
        </div>
