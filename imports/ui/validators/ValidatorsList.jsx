  ...
  render() {
    ...
    return <div id="validator-list">
      <Helmet>
        ...
        <meta name="description" content="Here is a list of Fetch.ai Validators" />
      </Helmet>
      <Row>
        ...
        <Col md={12}>
          <Card body>
            <Row className="header text-nowrap">
              ...
              {(!this.props.inactive) ? <Col className="uptime" md={4} onClick={(e) => this.toggleDir('uptime', e)}><i className="material-icons">flash_on</i> <span className="d-inline-block d-md-none d-lg-inline-block"><T>validators.uptime</T> ({Meteor.settings.public.slashingWindow} <i className="fas fa-cube"></i>)</span> {renderToggleIcon(this.state.uptimeDir == 1)}</Col> : ''}
