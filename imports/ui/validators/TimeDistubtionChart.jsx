...
const displayTimeRange = (time) => {
  const startTime = time.format('h:mm');
  const endTime = time.clone().add(BATCHSIZE, 'minute').format('h:mm');
...
export default class TimeDistubtionChart extends Component {
  populateChartData() {
    ...
    this.props.missedRecords.forEach((record) => {
      const time = moment.utc(record.time);
  ...
  populateTimelineChart(timeline) {
    const layout = [['yAxis', 'barPlot'], [null, 'xAxis']];
    const scales = [{
      scaleId: 'xScale',
      type: 'Time',
    }, {
