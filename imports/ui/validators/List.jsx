    ...
    return <Card body>
        <SentryBoundary><Row className="validator-info">
            ...
            <Col className="voting-power data" xs={{ size: 8, offset: 2 }} md={{ size: 3, offset: 0 }} lg={2}>
                ...
                <span>
                    {props.validator.tokens ? numbro(Big(props.validator.tokens).div(Meteor.settings.public.powerReduction).round(0, Big.roundDown).toString()).format('0') : 0}
                    &nbsp;&nbsp;({props.validator.tokens ? numbro(Big(props.validator.tokens).div(Meteor.settings.public.powerReduction).div(props.totalPower).toString()).format('0.00%') : "0.00%"})
                </span>
            </Col>
            ...
            {(!props.inactive) ? <Col className="commission data" xs={{ size: 4 }} md={{ size: 1, offset: 0 }} lg={2}><i className="material-icons d-sm-none">call_split</i> <span>{(props.validator.commission && props.validator.commission.commission_rates) ? numbro(props.validator.commission.commission_rates.rate).format('0.00%') : ''}</span></Col> : ''}
            {(!props.inactive) ? <Col className="uptime data" xs={{ size: 2, order: "last" }} md={4}><Progress animated value={props.validator.uptime}><span className="d-none d-md-inline">{props.validator.uptime ? props.validator.uptime.toFixed(2) : 0}%</span><span className="d-md-none">&nbsp;</span></Progress></Col> : ''}
