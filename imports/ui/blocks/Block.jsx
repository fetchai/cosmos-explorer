                    ...
                    <Card>
                      ...
                      <CardBody>
                        <Row>
                          ...
                          <Col md={8} className="value"><Link
                            to={'/validator/' + ((proposer) ? proposer.operator_address : '')}><Avatar moniker={moniker}
                          ...
                          <Col md={8} className="value"><TimeStamp time={block.time}/> ({moment(block.time).fromNow()})</Col>
                          {
                            this.showDKGInfo() ?
                              <>
                                ...
                                <Col md={4} className="label" onClick={this.copy}
                                     data-tooltip-positions="bottom;left;top;right"><span
                                  className={'DKG-value'}><T>DKG.groupSignature</T></span></Col>
                                <Col md={8}  data-tooltip-positions="bottom;left;top;right"
                                     className="value text-nowrap overflow-auto address">
                                  {format(block.dkg.groupSignature, 30)}
                                  <Tooltip title={this.state.copied ? <span className="toolTip">Copied!</span> : <span
                                    className="toolTip">Copy to Clipboard</span>}>
                                    <img onClick={this.copy} onClick src="/img/clipboard.svg" className="clipboard"/></Tooltip>

                                </Col>
                                <Col md={4} className="label" onClick={this.copy}
                                     data-tooltip-positions="bottom;left;top;right"><span
                                  className={'DKG-value'}><T>DKG.round</T></span></Col>
                                <Col md={8} className="value text-nowrap overflow-auto address">{block.dkg.round}</Col>
                                <Col md={4} className="label" onClick={this.copy}
                                     data-tooltip-positions="bottom;left;top;right"><span
                                  className={'DKG-value'}><T>DKG.startBlock</T></span></Col>
                                <Col md={8}
                                     className="value text-nowrap overflow-auto address">{block.dkg.startBlock}</Col>
                                <Col md={4} className="label" onClick={this.copy}
                                     data-tooltip-positions="bottom;left;top;right"><span
                                  className={'DKG-value'}><T>DKG.endBlock</T></span></Col>
                                <Col md={8}
                                     className="value text-nowrap overflow-auto address">{block.dkg.endBlock}</Col>
                              </>
                              : ''
                          }
                        </Row>
