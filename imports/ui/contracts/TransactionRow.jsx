  ...
  return (
    <SentryBoundary>
      <Row className={(tx.code) ? 'tx-info invalid' : 'tx-info'}>
        ...
        <Col xs={(!props.blockList) ? { size: 6, order: 'last' } : { size: 12, order: 'last' }} md={(!props.blockList) ? { size: 3, order: 'last' } : { size: 7, order: 'last' }} lg={(!props.blockList) ? { size: 2, order: 'last' } : { size: 3, order: 'last' }} className="text-truncate">
          <i className="fas fa-hashtag d-lg-none" />
          {' '}
          <Link to={`/contracts/${tx.txhash}`}>
            {tx.txhash}
          </Link>
        </Col>
