...
const maxHeightModifier = {
  setMaxHeight: {
    enabled: true,
    fn: (data) => ({ ...data, styles: { ...data.styles, overflowY: 'auto', maxHeight: '80vh' } }),
  },
};
...
const TypeMeta = {
  ...
  [Types.REDELEGATE]: {
    ...
    warning: (duration, maxEntries) => {
      const day = duration * durationToDay;
...
const CoinAmount = (props) => {
  ...
  const coin = new Coin(props.amount, props.denom).toString(4);
  const denom = (props.mint) ? Coin.StakingCoin.denom : Coin.StakingCoin.displayName;
  return (
    <span>
      <span className={props.className || 'coin'}>
        {coin}
      </span>
      {' '}
    </span>
...
const Amount = (props) => {
  if (!props.coin && !props.amount) return null;
  const coin = props.coin || new Coin(props.amount, props.denom).toString(4);
  const amount = (props.mint) ? Math.round(coin.amount) : coin.stakingAmount;
  const denom = (props.mint) ? Coin.StakingCoin.denom : Coin.StakingCoin.displayName;
  return (
    <span>
      <span className={props.className || 'amount'}>
        {numbro(amount).format('0,0.0000')}
      </span>
      {' '}
...
const Fee = (props) => (
  <span>
    <CoinAmount mint className="gas" amount={props.gas * Meteor.settings.public.gasPrice} />
    {' '}
    as fee
    {' '}
  </span>
...
const isActiveValidator = (validator) => !validator.jailed && validator.status === 2;
...
const isAddress = (address) => address && startsWith(address, Meteor.settings.public.bech32PrefixAccAddr);

const isValidatorAddress = (address) => address && startsWith(address, Meteor.settings.public.bech32PrefixValAddr);
  ...
  constructor(props) {
    ...
    componentDidUpdate(prevProps, prevState) {
      ...
      if ((this.state.isOpen && !prevState.isOpen) || (this.state.user && this.state.user != prevState.user)) {
        if (!this.state.success) { this.tryConnect(); }
        this.getBalance();
      }
    }
    ...
    autoOpenModal = () => {
      const query = this.props.history && this.props.history.location.search.substr(1);
    ...
    getBalance = () => {
      ...
      if (this.state.actionType === Types.REDELEGATE) {
        Meteor.call('accounts.getAllRedelegations', this.state.user, this.props.validator.operator_address, (error, result) => {
          try {
            if (result) { this.setStateOnSuccess('loadingRedelegations', { redelegations: result }); }
            if (!result || error) {
      ...
      Meteor.call('accounts.getAccountDetail', this.state.user, (error, result) => {
        try {
          if (result) {
            const coin = result.coins ? (new Coin(result.coins[0].amount, result.coins[0].denom)) : (new Coin(0, result.coins[0].denom));
    ...
    getPath = () => {
      const meta = TypeMeta[this.state.actionType];
    ...
    runSimulatation = (txMsg, simulateBody) => {
      const gasAdjustment = TypeMeta[this.state.actionType].gasAdjustment || DEFAULT_GAS_ADJUSTMENT;
    ...
    populateRedirectParams = () => ({ action: this.state.actionType })

    isDataValid = () => this.state.currentUser != undefined

    getActionButton = () => {
      if (this.state.activeTab === '0') { return <Button color="primary" onClick={this.redirectToSignin}>Sign in With Ledger</Button>; }
      if (this.state.activeTab === '1') { return <Button color="primary" onClick={this.tryConnect}>Continue</Button>; }
      ...
      if (this.state.activeTab === '3') {
        return (
          <Button color="primary" disabled={this.state.signing} onClick={this.sign}>
    ...
    getValidatorOptions = () => {
      const activeValidators = Validators.find(
        { jailed: false, status: 2 },
        { sort: { 'description.moniker': 1 } },
      );
      const redelegations = this.state.redelegations || {};
      const maxEntries = this.props.stakingParams.max_entries;
      return (
        <UncontrolledDropdown direction="down" size="sm" className="redelegate-validators">
          ...
          <DropdownMenu modifiers={maxHeightModifier}>
            {activeValidators.map((validator, i) => {
              ...
              const redelegation = redelegations[validator.operator_address];
              const disabled = redelegation && (redelegation.count >= maxEntries);
              const completionTime = disabled ? <TimeStamp time={redelegation.completionTime} /> : null;
              const id = `validator-option${i}`;
              return (
                <div id={id} className={`validator disabled-btn-wrapper${disabled ? ' disabled' : ''}`} key={i}>
                  <DropdownItem
                    ...
                    data-address={validator.operator_address}
                  >
                    <Row>
                      <Col xs="12" className="moniker">
    ...
    renderConfirmationTab = () => {
      ...
      return (
        <TabPane tabId="3">
          <div className="action-summary-message">
    ...
    renderModal = () => (
      <Modal isOpen={this.state.isOpen} toggle={this.close} className="ledger-modal">
        <ModalBody>
          <TabContent className="ledger-modal-tab" activeTab={this.state.activeTab}>
            ...
            <TabPane tabId="4">
              <div>
                Transaction is broadcasted.  Verify it at
                <Link to={`/transactions/${this.state.txHash}?new`}> transaction page. </Link>
              </div>
              <div>
                See your activities at
                <Link to={`/account/${this.state.user}`}>your account page</Link>
                .
              </div>
            </TabPane>
          </TabContent>
...
class DelegationButtons extends LedgerButton {
  constructor(props) {
    ...
    renderActionTab = () => {
      ...
      const moniker = this.props.validator.description && this.props.validator.description.moniker;
      const validatorAddress = <span className="ellipic">this.props.validator.operator_address</span>;
      switch (this.state.actionType) {
      case Types.DELEGATE:
        action = 'Delegate to';
        maxAmount = this.state.currentUser.availableCoin;
    ...
    getWarningMessage = () => {
      const duration = this.props.stakingParams.unbonding_time;
      const maxEntries = this.props.stakingParams.max_entries;
      const { warning } = TypeMeta[this.state.actionType];
    ...
    getConfirmationMessage = () => {
      ...
      case Types.DELEGATE:
        return (
          <span>
            ...
            {' '}
            <Amount coin={this.state.delegateAmount} />
            {' '}
            to
            {' '}
            <AccountTooltip address={this.props.validator.operator_address} sync />
            {' '}
            with
            {' '}
            <Fee gas={this.state.gasEstimate} />
            .
          </span>
        );
      case Types.REDELEGATE:
        return (
          <span>
            You are going to
            <span className="action">redelegate</span>
            {' '}
            <Amount coin={this.state.delegateAmount} />
            {' '}
            from
            ...
            to
            {' '}
            <AccountTooltip address={this.state.targetValidator && this.state.targetValidator.operator_address} sync />
            {' '}
            with
            {' '}
            <Fee gas={this.state.gasEstimate} />
      ...
      case Types.UNDELEGATE:
        return (
          <span>
            You are going to
            <span className="action">undelegate</span>
            {' '}
            <Amount coin={this.state.delegateAmount} />
            {' '}
            from
    ...
    renderRedelegateButtons = () => {
      const delegation = this.props.currentDelegation;
      if (!delegation) return null;
      const completionTime = delegation.redelegationCompletionTime;
      const isCompleted = !completionTime || new Date() >= completionTime;
      const maxEntries = this.props.stakingParams.max_entries;
      const canUnbond = !delegation.unbonding || maxEntries > delegation.unbonding;
      return (
        <span>
          <div id="redelegate-button" className={`disabled-btn-wrapper${isCompleted ? '' : ' disabled'}`}>
            ...
            {isCompleted ? null : (
              <UncontrolledTooltip placement="bottom" target="redelegate-button">
                <span>
                  You have incompleted regelegation to this validator,
                  you can't redelegate until
                  <TimeStamp time={completionTime} />
                </span>
              </UncontrolledTooltip>
            )}
          </div>
          <div id="undelegate-button" className={`disabled-btn-wrapper${canUnbond ? '' : ' disabled'}`}>
            ...
            {canUnbond ? null : (
              <UncontrolledTooltip placement="bottom" target="undelegate-button">
                <span>
                  You reached maximum
                  {maxEntries}
                  {' '}
                  unbonding delegation entries,
                  you can&apos;t delegate until the first one matures at
                  <TimeStamp time={delegation.unbondingCompletionTime} />
                </span>
              </UncontrolledTooltip>
            )}
          </div>
    ...
    render = () => (
      <span className="ledger-buttons-group float-right">
        {isActiveValidator(this.props.validator) ? (
          <Button
            color="success"
            size="sm"
            onClick={() => this.openModal(Types.DELEGATE)}
          >
...
class TransferButton extends LedgerButton {
    renderActionTab = () => {
      if (!this.state.currentUser) return null;
      const maxAmount = this.state.currentUser.availableCoin;
      return (
        <TabPane tabId="2">
          <h3>
            Transfer
    ...
    getConfirmationMessage = () => (
      <span>
        ...
        {' '}
        <Amount coin={this.state.transferAmount} />
        {' '}
        to
        {' '}
        {this.state.transferTarget}
    ...
    render = () => {
      let params = {};
      let { button } = TypeMeta[Types.SEND];
      ...
      return (
        <span className="ledger-buttons-group float-right">
          <Button color="info" size="sm" onClick={() => this.openModal(Types.SEND, params)}>
            {' '}
            {button}
            {' '}
          </Button>
...
class SubmitProposalButton extends LedgerButton {
    ...
    getPath = () => TypeMeta[Types.SUBMITPROPOSAL].path
    ...
    isDataValid = () => {
      if (!this.state.currentUser) return false;
      return (
        this.state.proposalTitle != null
          && this.state.proposalDescription != null
          && isBetween(this.state.depositAmount, 1, this.state.currentUser.availableCoin)
      );
    }
    ...
    getConfirmationMessage = () => (
      <span>
        You are going to
        <span className="action">submit</span>
        {' '}
        a new proposal.
        <div>
          <h3>
    ...
    render = () => (
      <span className="ledger-buttons-group float-right">
        <Button color="info" size="sm" onClick={() => this.openModal(Types.SUBMITPROPOSAL, {})}>
          {' '}
          {TypeMeta[Types.SUBMITPROPOSAL].button}
          {' '}
        </Button>
        {this.renderModal()}
      </span>
...
class ProposalActionButtons extends LedgerButton {
    renderActionTab = () => {
      ...
      case Types.VOTE:
        ...
        inputs = (
          <Input type="select" name="voteOption" onChange={this.handleInputChange} defaultValue="">
    ...
    getPath = () => {
      const { pathPreFix, pathSuffix } = TypeMeta[this.state.actionType];
    ...
    getConfirmationMessage = () => {
      ...
      case Types.VOTE:
        return (
          <span>
            You are
            <span className="action">voting</span>
            {' '}
            <strong>
              {this.state.voteOption}
            </strong>
      ...
      case Types.DEPOSIT:
        return (
          <span>
            You are
            <span className="action">deposit</span>
            {' '}
            <Amount coin={this.state.depositAmount} />
            {' '}
            to proposal
