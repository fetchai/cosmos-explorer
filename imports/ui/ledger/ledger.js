...
const REQUIRED_COSMOS_APP_VERSION =
  Meteor.settings.public.ledger.ledgerAppVersion || "2.16.0";
const DEFAULT_DENOM = Meteor.settings.public.bondDenom || "uatom";
export const DEFAULT_GAS_PRICE =
  parseFloat(Meteor.settings.public.ledger.gasPrice) || 0.025;
...
export class Ledger {
  ...
  async isCosmosAppOpen(transportBLE) {
    ...
    if (
      appName.toLowerCase() !==
      Meteor.settings.public.ledger.appName.toLowerCase()
    ) {
      throw new Error(
        `Close ${appName} and open the ${Meteor.settings.public.ledger.appName} app`
      );
    }
  }
  ...
  checkLedgerErrors(
    ...
    switch (error_message) {
      ...
      case `${Meteor.settings.public.ledger.appName} app does not seem to be open`:
        ...
        throw new Error(
          `${Meteor.settings.public.ledger.appName} app is not open`
        );
      ...
      case `Web Bluetooth API globally disabled`:
        throw new Error(
          `Bluetooth not supported. Please use the latest version of Chrome browser.`
        );
      case `No errors`:
        // do nothing
        break;
  ...
  static createDelegate(txContext, validatorBech32, uatomAmount) {
    const txMsg = {
  ...
  // the function expects a complete txContext
  static createUndelegate(txContext, validatorBech32, uatomAmount) {
    const txMsg = {
  ...
  // the function expects a complete txContext
  static createTransfer(txContext, toAddress, amount) {
    const txMsg = {
  ...
  static createVote(txContext, proposalId, option) {
    return {
  ...
  static createDeposit(txContext, proposalId, amount) {
    const txMsg = {
  ...
  async signAmino(signerAddress, signDoc) {
    await this.connect(INTERACTION_TIMEOUT, false);

    const account = await this.getCosmosAddress(this.cosmosApp);
...
function canonicalizeJson(jsonTx) {
  ...
  Object.keys(jsonTx)
    .sort()
    .forEach((key) => {
      if (typeof jsonTx[key] == "object") {
        jsonTx[key] = canonicalizeJson(jsonTx[key]);
      }
