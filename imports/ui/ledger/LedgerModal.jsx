...
import {
  Button, Modal, ModalBody, ModalFooter, ModalHeader, Spinner, TabContent, TabPane,
} from 'reactstrap';
  ...
  constructor(props) {
    ...
    tryConnect = (timeout = undefined) => {
      ...
      this.ledger.getCosmosAddress(timeout).then((res) => {
        const currentUser = localStorage.getItem(CURRENTUSERADDR);
    ...
    getActionButton() {
      if (this.state.activeTab === '1' && !this.state.loading) {
        return (
          <Button color="primary" onClick={this.tryConnect}>
            <T>common.retry</T>
          </Button>
        );
      }
      if (this.state.activeTab === '2' && this.state.errorMessage !== '') {
        return (
          <Button color="primary" onClick={this.trySignIn}>
            <T>common.retry</T>
          </Button>
        );
      }
          ...
          <ModalBody>
            <TabContent activeTab={this.state.activeTab}>
              ...
              <TabPane tabId="2">
                ...
                <T>accounts.toLoginAs</T>
                {' '}
                <strong className="text-primary d-block">
                  {this.state.address}
                </strong>
                <T>accounts.pleaseAccept</T>
              </TabPane>
            </TabContent>
