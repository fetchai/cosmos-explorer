...
export default class Coin {
    ...
    toString(precision) {
        ...
        if (this.amount < minStake) {
            return `${numbro(this.amount)
                .format('0,0')} ${this._coin.denom}`;
        } else {
            return `${precision ? numbro(this.stakingAmount)
                .format('0,0.' + '0'.repeat(precision)) : autoformat(this.stakingAmount)} ${this._coin.displayName}`
        }
    }
