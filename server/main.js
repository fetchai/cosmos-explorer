...
getProposals = () => {
  Meteor.call("proposals.getProposals", (error, result) => {
    if (error) {
      console.log("get proposal error: %o", error);
    }
    if (result) {
...
getProposalsResults = () => {
  Meteor.call("proposals.getProposalResults", (error, result) => {
    if (error) {
      console.log("get proposals result error: %o", error);
    }
    if (result) {
...
aggregateMinutely = () => {
  ...
  Meteor.call(
    "Analytics.aggregateBlockTimeAndVotingPower",
    "m",
    (error, result) => {
      if (error) {
    ...
    Object.keys(DEFAULTSETTINGSJSON).forEach((key) => {
      ...
      Object.keys(DEFAULTSETTINGSJSON[key]).forEach((param) => {
        if (Meteor.settings[key][param] == undefined) {
          console.warn(
            `CHECK SETTINGS JSON: ${key}.${param} is missing from settings`
          );
